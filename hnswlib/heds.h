...
class HEDS : public AlgorithmInterface<dist_t> {
 public:
    ...
    pgm::PGMIndex<float, 8> Shortcuts;
    std::vector<std::pair<dist_t, tableint>> densityLevel;
    dist_t dis_bound;
    ...
    dist_t getDisByLevel(tableint data1, tableint data2, int level) const {
        ...
        if(level == 0) return fstdistfunc_(getDataByInternalId(data1), getDataByInternalId(data2), dist_func_param_);
    ...
    std::pair<dist_t,dist_t> getDensityByLevel(tableint curData ,const void *data_point, labeltype label, int level){

        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates = 
        searchBaseLayer(curData, data_point, label, level);
        //getNeighborsByHeuristic2(top_candidates, M_*2, level);
        ...
        while(top_candidates.size()){
            avg_dis += top_candidates.top().first;
            if(top_candidates.top().first > 0 && Vd > 0.0001){
                float dis_ = top_candidates.top().first;
                float dis_d = dis_;
                for(int i =1 ; i<= d; i++) dis_d = dis_d * dis_;

                density_k += k_index/(max_elements_*Vd*dis_d);
                //std::cout << k_index << " " << max_elements_ << " " << Vd << " " << dis_d << std::endl;
                k_index--;
            }
            top_candidates.pop();
        }
    ...
    //std::vector<std::pair<dist_t, tableint>> buildShortcuts(){
    void buildShortcuts(int &num_base){
        ...
        for (int i = 0; i< num_base; i++) {
            ...
            //std::cout<< cur_point << ":" << point_level << "   ";
            int skipLevel = 1;
            int skip_to_level = point_level -2;
            if(point_level > 1){
                
                auto curlevel_result = getDensityByLevel(cur_point,getDataByInternalId(cur_point),cur_point,point_level);
                float curlevel_density = curlevel_result.first;

                while(skip_to_level >= 0){
                    
                    auto skiplevel_result = getDensityByLevel(cur_point,getDataByInternalId(cur_point),cur_point,skip_to_level);
                    float skiplevel_density = skiplevel_result.first;

                    if(skiplevel_density >= curlevel_density) skipLevel++;
                    else break;
                ...
                if(skipLevel > 1 || density_skipLevels.size() < ef_){
                    density_skipLevels.emplace(-curlevel_result.second, skipLevel);
                }
            }
            //cur_point ++;
        }
        ...
        while(density_skipLevels.size()){
            ...
            densityLevel.push_back(std::make_pair(-density_skipLevels.top().first, density_skipLevels.top().second));
            denseDensity.push_back(-density_skipLevels.top().first);
            density_skipLevels.pop();
        }
        //std::cout<<"Finish building shortcuts" <<std::endl;
        //Build shortcuts
        dis_bound = densityLevel[densityLevel.size()-1].first;

        std::sort(denseDensity.begin(), denseDensity.end());
        auto last = std::unique(denseDensity.begin(), denseDensity.end());
        denseDensity.erase(last, denseDensity.end());
        std::cout << "denseDensity Size: " << denseDensity.size() << std::endl;
        pgm::PGMIndex<float, 8> index(denseDensity);
        Shortcuts = index;
   
    }

    //Given the entry data, query data, current level and calculate dis, then return the skipped level
    int levelsSkip(tableint curData, tableint query_Id, int level) const {
        //float cur_density = getDensityByLevel(curData, getDataByInternalId(curData), curData, level);
        
        dist_t dis = getDisByLevel(curData, query_Id, level);
        if(dis > dis_bound) return 1;
        auto range = Shortcuts.search(dis);

        return densityLevel[range.pos].second;
    }

    std::priority_queue<std::pair<dist_t, labeltype >>
    searchKnnShortcuts(Query &query, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        for(int level = maxlevel_; level > 0;){
            //dist_t curdist = getDisByLevel(getDataRepreByLevel(query_Id,level), getDataRepreByLevel(currObj,level), level);
            ...
            int skip_to_level = level - levelsSkip(currObj, query.query_Id, level);
        ...
        if (bare_bone_search) {
            top_candidates = searchBaseLayerST<true>(
                    currObj, getDataByInternalId(query.query_Id), (size_t)(query.k*1.2), isIdAllowed);
        } else {
    ...
    searchBaseLayer(tableint ep_id, const void *data_point, labeltype label, int layer) {
        ...
        if (!isMarkedDeleted(ep_id)) {
            ...
            dist_t dist = getDisByLevel(label, ep_id, layer);
...
//                    if (candidate_id == 0) continue;
                ...
                dist_t dist1 = getDisByLevel(label, candidate_id, layer);
    ...
    void getNeighborsByHeuristic2(
        ...
        while (queue_closest.size()) {
            ...
            for (std::pair<dist_t, tableint> second_pair : return_list) {
                ...
                dist_t curdist = getDisByLevel(second_pair.second, curent_pair.second, level);
        ...
        for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
            ...
            if (!is_cur_c_present) {
                if (sz_link_list_other < Mcurmax) {
                    ...
                    dist_t d_max = getDisByLevel(cur_c, selectedNeighbors[idx], level);
        ...
        if ((signed)currObj != -1) {
                ...
                for (int level = maxlevelcopy; level > curlevel; level--) {
                    ...
                    while (changed) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            dist_t d = getDisByLevel(cur_c, cand, level);

                            //std::cout << " ------------------------------------------------------------------ " <<std::endl;
                            //int rep_size = getDatasizeLevel(level);
                            //float *pVect1 = (float *) getDataRepreByLevel(cur_c, level);
                            //float *pVect2 = (float *) getDataRepreByLevel(cand, level);
            ...
            for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                ...
                if (epDeleted) {
                    ...
                    top_candidates.emplace(getDisByLevel(label, enterpoint_copy, level), enterpoint_copy);
                    if (top_candidates.size() > ef_construction_)
                        top_candidates.pop();
                }
